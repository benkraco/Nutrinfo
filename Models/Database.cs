...
public static class Database
{
    ...
    public static List<Productos> ListarProductos()
    {
        ...
        using (SqlConnection connection = new SqlConnection(_connectionString))
        {
            listaProductos = connection.Query<Productos>("SELECT TOP 10 * FROM Productos").ToList();
        }
    ...
    public static List<Productos> BuscarProductos(string busqueda)
    {
        ...
        using (SqlConnection connection = new SqlConnection(_connectionString))
        {
            listaProductos = connection.Query<Productos>("SELECT * FROM Productos WHERE Nombre LIKE @pBusqueda", new { pBusqueda = "%" + busqueda + "%" }).ToList();
        }
